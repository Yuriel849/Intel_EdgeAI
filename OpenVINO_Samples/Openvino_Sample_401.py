...
urllib.request.urlretrieve( # Acquire this file of useful utility functions from online
...
import cv2
import numpy as np
from IPython import display # 'IPython' only required in Jupyter
...
# Unpack the model
tf_model_path = base_model_dir / archive_name.with_suffix("").stem / "frozen_inference_graph.pb" # Tensorflow models use '.pb' extension
if not tf_model_path.exists():
    with tarfile.open(downloaded_model_path) as file:
        file.extractall(base_model_dir)

precision = "FP16"
# FP16 is faster but less accurate (lower precision) than FP32
# OpenVINO's IR XML file is not much affected, but the IR bin file is much smaller with FP16
# Use FP16 for faster inference, FP32 for greater precision
...
# bin file detected and processed without being specified in read_model()
# bin file assumed to be in same location as XML file
